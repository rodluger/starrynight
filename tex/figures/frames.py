...
add_basis(ax, r"$\mathbf{\tilde{y}}$")
add_frame(ax, r"$\mathcal{F}\mathbf{''}$")
...
ax.text(
    0.65, 0.5, r"$f_0$", transform=ax.transAxes, size=14, ha="center", va="center",
)
...
ax.imshow(fgreens, **flt_kw)
plot_grid(ax, inc, obl_greens, theta)
add_basis(ax, r"$\mathbf{\tilde{g}}$")
add_frame(ax, r"$\mathcal{F}\mathbf{'}$")
add_terminator(ax, b, theta)
add_occultor(ax, 0, bo, ro)
axes.append(ax)

ax = plt.subplot2grid((3, 7), (0, 0))
ax.text(
...
add_arrow(axes[1], axes[2], r"$\mathbf{R}\mathbf{''}$", kind="bar_down")
add_arrow(axes[2], axes[3], r"$\mathbf{A_1}$")
add_arrow(axes[3], axes[4], r"$\mathbf{I}$")
add_arrow(axes[4], axes[5], r"$\mathbf{\mathbb{r}}^\top$", eps=0.2)
add_arrow(axes[1], axes[6], r"$\mathbf{R}\mathbf{'}$", kind="bar_up")
add_arrow(axes[6], axes[7], r"$\mathbf{A_1}$")
add_arrow(axes[7], axes[8], r"$\mathbf{I}$")
add_arrow(axes[8], axes[9], r"$\mathbf{A_2}$")
add_arrow(axes[9], axes[10], r"$\mathbf{\mathbb{s}}^\top$", eps=0.2)
