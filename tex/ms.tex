...
are computed (Equation~\ref{eq:A1}),
%
...
Recalling the structure of the basis (Equation~\ref{eq:p}),
%
\begin{align}
    \bp & =
    \begin{pmatrix}
...
\end{align}
%
recalling that in frame $\mathcal{F}_\mathrm{int}$, $\theta = 0$ by
definition.
%
...
occultor position. In the following sections, we present the solution for the
$\sT$, and hence for the total visible flux, for each of these cases.

\xxx{How do we determine which case we're modeling?}
