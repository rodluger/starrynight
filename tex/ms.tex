...
intersects with the limb). We compute this by first weighting the solution
for the occultation in emitted light by the intensity profile as in
case 3. The flux $f_3$ obtained in this fashion contains the contribution
from the entire nightside, which as we discuss above is unphysically weighted
by the negative part of Equation~(\ref{eq:illum_poly}). We can compute what
this contribution is from the complement of case 0:
%
\begin{proof}{}
    \label{eq:f0hat}
    \hat{f}_0 & =
...
\end{proof}
%
i.e., the flux we would get if we placed a negative illumination source
at the antipode $(-x_s, -y_s, -z_s)$ of the true source.
The total flux in this case is thus
%
\begin{proof}{}
    \label{eq:f4}
    f_4 = f_3 - \hat{f}_0
...
\begin{figure}[t!]
    \begin{centering}
        ...
        \oscaption{geometry}{%
            ...
            the occulted portion of the dayside (light gray region)
...
\end{align}
%
Note that this is not necessarily the \emph{observed} flux, which we must
...
the terminator. Case 6 corresponds to occultors that block some of the
nightside and some of the dayside, regardless of whether the occultor
is fully within the disk of the occulted body. We begin by computing the
...
\begin{proof}{}
    \label{eq:f6}
    f_6 = f_3 - (\hat{f}_0 - f_S)
    \quad.
\end{proof}
%
Case 7 corresponds to occultors that \xxx{what?}
%
\begin{proof}{}
    \label{eq:7}
    f_7 = f_3 - f_S
...
\end{proof}
%
Finally, case 8 corresponds to occultors that \xxx{what?}
%
\begin{proof}{}
    \label{eq:f8}
    f_8 = f_S
...
%
Pathological cases with 3 or 4 roots here.

%

\subsection{Computing the integrals $\sT$}
\label{sec:sT}
\xxx{This is going to be be painful.}
