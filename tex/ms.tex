...
and $\vartheta_i$ is the angle between the incident radiation and the surface
normal.
%
...
body of unit radius and unit spherical albedo may be approximated as
%
\begin{align}
    \mathcal{I} & = \frac{1}{\pi r_s^2}
    \text{max}\Big(0 \, ,
    \cos \vartheta_i\Big)
...
\\[1em]
%
and $r_s$ is the distance to the illumination source, whose flux measured by
the observer is unity.
The expression above is based on Equation~(30) in \citet{OrenNayar1994}. At
a given point on the surface, this intensity is a function of a single
...
The angles $\vartheta_i$, $\vartheta_r$, $\phi_i$, $\phi_r$ are all
implicit functions of $x$, $y$, and the source position.
\xxx{TODO.}
