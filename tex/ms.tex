...
\xxx{
    \textbf{To do list:}
    \begin{itemize}
        \item Make our frame convention explicit.
              Unprimed = sky. Prime = greens. Double prime = term.
        \item Emphasize why we can't just analytically weight the nightside by
              zero and integrate as before.
        \item Add a table clarifying the notation.
    \end{itemize}
}
...
%
\begin{align}
    \phi_0 & = \frac{\pi}{2} \pm \left(\arcsin\left(\frac{1 - r_o ^ 2 - b_o ^ 2}{2 b_o r_o}\right) - \frac{\pi}{2}\right)
    \quad,
\end{align}
%
...
incomplete elliptic integrals $\mathbbb{F}(k^2, \vkappa)$ and
$\mathbbb{E}(k^2, \vkappa)$,
...
\begin{proof}{J}
    ...
    \nonumber\\
    %
    \mathbf{z}_1(k^2, \pmb{\kappa}) & =
    \left(\left(3 \sinhalfkap[2] + 4\right) - 6k^2\right)
    \mathbf{z}_0(k^2, \pmb{\kappa})
    %
    \nonumber\\
    %
    \mathbf{z}_v(k^2, \pmb{\kappa}) & =
    k^2
    \sinhalfkap[2v - 3]
    \coshalfkap
    \mathbf{q}(k^2, \pmb{\kappa})^5
    \quad,
\end{proof}
%
with
...
\begin{proof}{U}
    \label{eq:Usol}
    \iU_v(\vkappa) &= \frac{\Delta \sinhalfkap[v+1]}{v + 1}
    \quad.
\end{proof}
%
...
%
\begin{proof}{W}
    \iW_v(k^2, \vkappa) = \Delta \mathbf{b}_v(k^2, \vkappa)
    \quad.
\end{proof}
%
...
%
\begin{proof}{W}
    \mathbf{b}_0(k^2, \vkappa) &=
    \frac{\sinhalfkap[2]}{5}
    \left(
    \frac{1 - \mathbf{q}(k^2, \vkappa)^3}{1 - \mathbf{q}(k^2, \vkappa)^2}
    +
    \mathbf{q}(k^2, \vkappa)^3
    \right)
    \nonumber \\
    \mathbf{c}_0(k^2, \vkappa) & =
    \sinhalfkap[4]
    \frac{\mathbf{q}(k^2, \vkappa)^5}{1 - \mathbf{q}(k^2, \vkappa)^2}
    \quad,
\end{proof}
%
...
%
\begin{proof}{W}
    \mathbf{b}_v(k^2, \vkappa) & =
    \frac{1}{2v + 5}
    \left(
    \frac{2 v \sinhalfkap[2]}{1 - \mathbf{q}(k^2, \vkappa)^2}
    \mathbf{b}_{v - 1}(k^2, \vkappa)
    - \mathbf{c}_{v - 1}(k^2, \vkappa)
    \right)
    \nonumber
    \\
    \mathbf{c}_v(k^2, \vkappa) & =
    \sinhalfkap[2] \mathbf{c}_{v - 1}(k^2, \vkappa)
\end{proof}
%
for $v > 0$.
%
...
\begin{proof}{W}
    \mathbf{b}_\vmax &=
    \frac{\sin^{2\vmax + 2}\left(\frac{\vkappa}{2}\right)}{4\vmax + 10}
    \left(
    \mathbf{f}_\vmax(k^2, \vkappa)
    + 2 \mathbf{q}(k^2, \vkappa)^3
    \right)
    \nonumber \\
    \mathbf{c}_\vmax &=
    \frac{1}{2}
    \mathbf{q}(k^2, \vkappa)^5
    \sin^{2\vmax}\left(\frac{\vkappa}{2}\right)
    \quad,
\end{proof}
%
where
%
\begin{proof}{W}
    \mathbf{f}_v(k^2, \vkappa)
    &\equiv
    \frac{3}{v+1}
    \,
    {_2\mathbbb{F}_1}\left(
    -\frac{1}{2},
    v + 1;
    v + 2;
    1 - \mathbf{q}(k^2, \vkappa)^2
    \right)
\end{proof}
%
and ${_2\mathbbb{F}_1}(a, b; c; \mathbf{z})$ is the Gauss
...
\begin{proof}{W}
    \mathbf{b}_v(k^2, \vkappa) & =
    \frac{1 - \mathbf{q}(k^2, \vkappa)^2}{\sinhalfkap[2]}
    \left(1 + \frac{5}{2v + 2}\right)
    \mathbf{b}_{v + 1}(k^2, \vkappa) +
    \frac{\mathbf{c}_{v + 1}(k^2, \vkappa)}{v + 1}
    \nonumber
    \\
    \mathbf{c}_v(k^2, \vkappa) & =
    \frac{\mathbf{c}_{v + 1}(k^2, \vkappa)}{\sinhalfkap[2]}
    \quad.
\end{proof}
...
%
\begin{proof}{}
    \pmb{\xi_0} &= \atantwo\left(\sqrt{1 - {\mathbf{x''}}^2}, \mathbf{x''} \right)
\end{proof}
%
where $\mathbf{x''}$ are the roots of the quartic (Equation~\ref{eq:quartic}).
...
\end{proof}
%
As before, these angles are then sorted and placed in the vector
...
measured between the $x$-axis and a point on the curve of $\mathcal{T}$.
...
\begin{proof}{H}
    \label{eq:Hlower}
    \mathcal{H}_{0,0}(\vlambda) &= \Delta \vlambda
    %
    \nonumber \\
    %
    \mathcal{H}_{1,0}(\vlambda) &= \Delta \sin\vlambda
    %
    \nonumber \\
    %
    \mathcal{H}_{0,1}(\vlambda) &= -\Delta \cos\vlambda
    %
    \nonumber \\
    %
    \mathcal{H}_{1,1}(\vlambda) &= -\frac{\Delta\cos^2\vlambda}{2}
    %
    \quad.
\end{proof}
%
...
\begin{proof}{}
    \mathcal{\mathcal{T}}_{2}(b, \vxi) &= -\sgn(b) \Delta \mathbf{F}(b, \vxi)
\end{proof}
%
where
%
