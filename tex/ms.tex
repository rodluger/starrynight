...
\begin{figure}[h!]
    \begin{centering}
        ...
        \oscaption{speed}{%
            ...
            for a single flux evaluation as a function of spherical
            harmonic degree for different methods.
            In orange we show results for the emitted light
            ...
            dashed: with gradient). For comparison, in purple we show
            ...
            evaluations. The reflected light algorithm is around one
            order of magnitude slower and less precise than the emitted
            light algorithm, but still ${\sim}4$ orders of magnitude faster
...
\begin{align}
    \label{eq:F}
    \mathbb{F}(k^2, \kappa) & = \int_0^{\kappa} \frac{\dd \varphi}{\sqrt{1 - k^2 \sin^2 \varphi}}
    \\
    \intertext{and}
    \label{eq:E}
    \mathbb{E}(k^2, \kappa) & = \int_0^{\kappa} \sqrt{1 - k^2 \sin^2 \varphi} \, \dd \varphi
\end{align}
%
...
\begin{align}
    ...
    \vint{\pmb{\phi}}{
        \left(
        \frac{
            1 - (1 - r_o^2 - b_o^2 - 2 b_o r_o \sin\varphi)^\frac{3}{2}
        }{
            r_o^2 + b_o^2 + 2 b_o r_o \sin\varphi
        }
        \right)
...
\begin{proof}{}
    ...
    \Big(
    c_0 +
    c_1 \Delta\mathbbb{F}(k^2, \vkappa) +
    c_2 \Delta\mathbbb{E}(k^2, \vkappa) +
    c_3 \Delta\mathbbb{P}(k^2, \vkappa)
    \Big)
\end{proof}
%
where
...
\begin{proof}{}
    ...
    c_1 &= ?
    \nonumber \\
    c_2 &= -\frac{2}{3} \sqrt{k^2 b_o r_o} (4 - 7 r_o^2 - b_o^2)
    \nonumber \\
    c_3 &= \frac{1}{3} \sqrt{k^2 b_o r_o} \frac{b_o + r_o}{(b_o - r_o)^2}
\end{proof}
%
...
\end{proof}
%
The components of the vector $\viH$ are given by the integral
%
\begin{align}
    \label{eq:H}
    \iH_{u,v}(\vxi) & =
...
\end{proof}
%
where the vector $\viH$ is given by Equation~(\ref{eq:H}).
