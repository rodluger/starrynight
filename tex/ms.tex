...
the surface intensity field of the body is described by the spherical harmonic
coefficient vector $\mathbf{y}$. Following \citet{Luger2019}, we may compute
the total visible flux $f$ from this body as
%
\begin{align}
    \label{eq:sTARRy}
...
is a Wigner rotation matrix that
rotates $\bvec{y}$ to the sky frame $\mathcal{F}_\mathrm{sky}$
given the body's inclination $i$, obliquity $\lambda$, and
rotational phase $\vartheta$,
%
$\BF{R}'$ rotates the body on the plane
of the sky into the frame $\mathcal{F}_\mathrm{int}$ in which the
integration is actually performed,
%
$\BF{A}$ is the change-of-basis matrix from spherical harmonics
to the basis in which the integrals are computed
%,
and $\sT$ is the \emph{solution vector}, the integral of each
of the basis functions over
the projected visible disk of the body (which may be partially occulted by
another spherical body). For more details, see Appendix~\ref{app:starry}.
...
vector $\sT$, which are surface integrals over
...
day/night terminator of the occulted body.
