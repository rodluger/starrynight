...
\xxx{A few paragraphs of introduction here.
    ...
    \citet{Farr2018}, \citet{Louden2018}.}
...
computed from the JPL Horizons database \xxx{(cite)}
...
this within the \starry framework. Another example is specular reflection,
...
\begin{align}
    \label{eq:PInv}
    \mathbf{P}^+ =
    (\mathbf{P}^\top \mathbf{P} + \lambda \mathbf{I})^{-1} \mathbf{P}^\top
\end{align}
%
is the pseudoinverse of $\mathbf{P}$, with (small) regularization parameter
$\lambda$ and where $\mathbf{I}$ is the identity matrix.
%
...
computation; in practice, we find that grids with $2-5$ times as many pixels
as spherical harmonic coefficients are ideal.
%
